...
(async()=>{for(const engine of ['chromium','webkit']){
 ...
 try{for(const width of (process.env.AUTH_WIDTHS||'320,360,390,430,768,1280').split(',').map(Number)){
  ...
  await test('Reset uses existing recovery endpoint and generic response',async()=>{const r=await p.evaluate(()=>authTest.calls.find(x=>x.name==='reset'));assert(r.args.redirectTo.endsWith('?reset=1'));assert.match(await p.locator('#authRecoverySentCopy').innerText(),/Si existe una cuenta/);await p.getByRole('button',{name:'Volver a iniciar sesión',exact:true}).click()});
