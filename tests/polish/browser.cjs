...
(async()=>{for(const engine of ['chromium','webkit']){
 ...
 await test(role+' settings, default System, live OS and explicit theme persistence',async()=>{
  await go(role);await theme('system');await p.locator('#app .settings-trigger').click();assert.equal(await p.locator('#settingsRole').innerText(),role==='client'?'Atleta':'Entrenador');assert.equal(await p.locator('#settingsDialog select').count(),0);assert.equal(await p.locator('#settingsDialog a').count(),0);assert.equal(await p.locator('#settingsDialog input:checked').inputValue(),'system');
  await p.emulateMedia({colorScheme:'dark'});await p.waitForFunction(()=>document.documentElement.dataset.theme==='dark');assert.equal(await p.locator('html').getAttribute('data-theme'),'dark');
