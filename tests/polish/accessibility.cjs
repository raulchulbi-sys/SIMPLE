...
(async()=>{for(const engine of ['chromium','webkit']){const b=await(engine==='webkit'?webkit:chromium).launch({headless:true,...(engine==='chromium'?{channel:'msedge'}:{})});try{for(const width of [320,1280]){
 const context=await b.newContext({viewport:{width,height:900},colorScheme:'light'}),p=await context.newPage();p.setDefaultTimeout(5000);await p.goto('http://127.0.0.1:4182/?role=client');await p.waitForFunction(()=>window.previewReady);
 ...
 await test('Native keyboard radios, focus trap, Escape, real storage event and reopen preserve preference',async()=>{
  await p.evaluate(()=>simpleTheme.set('system'));await p.locator('#app .settings-trigger').click();await p.locator('input[value=system]').focus();await p.keyboard.press('ArrowRight');assert.equal(await p.evaluate(()=>simpleTheme.choice),'light');await p.keyboard.press('ArrowRight');assert.equal(await p.evaluate(()=>simpleTheme.choice),'dark');assert.equal(await p.locator('input[value=dark]').evaluate(e=>getComputedStyle(e.parentElement).outlineStyle),'solid');
  ...
  const q=await p.context().newPage();await q.goto('http://127.0.0.1:4182/?role=client');await q.waitForFunction(()=>window.previewReady);assert.equal(await q.evaluate(()=>simpleTheme.choice),'dark');await q.evaluate(()=>simpleTheme.set('light'));await p.waitForFunction(()=>simpleTheme.choice==='light');await q.close();
  const reopened=await p.context().newPage();await reopened.goto('http://127.0.0.1:4182/?role=trainer');await reopened.waitForFunction(()=>window.previewReady);assert.equal(await reopened.evaluate(()=>simpleTheme.choice),'light');await reopened.close();
 });
