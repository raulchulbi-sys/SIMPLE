...
const dir=require('path').resolve(__dirname,'../cycle-alias'),c=JSON.parse(fs.readFileSync(dir+'/private/config.json'));
assert.equal(c.ref,'dmqjexigdnfzobarhnib');const url='https://'+c.ref+'.supabase.co',file=__dirname+'/results/authenticated.json';
const report=process.argv[2]!=='suite'&&fs.existsSync(file)?JSON.parse(fs.readFileSync(file)):{project:c.ref,results:[],trace:[]};
...
async function main(){const action=process.argv[2];
 ...
 await scenario('A no workouts',[],5,0);
 await scenario('B partial days 1,2,3',[day(0,3),day(1,2),day(2,1)],5,3);
 await scenario('C inactivity never resets partial',[day(0,15),day(1,14),day(2,13)],5,3);
 await scenario('D repeat first starts next cycle',[day(0,8),day(1,7),day(2,6),day(0)],5,1);
 await scenario('E next cycle second day',[day(0,8),day(1,7),day(2,6),day(0,1),day(1)],5,2);
 await scenario('F skipped day leaves 4 of 5',[day(0,4),day(1,3),day(3,2),day(4,1)],5,4);
 await scenario('G skipped day then repeat starts cycle',[day(0,8),day(1,7),day(3,6),day(4,5),day(0)],5,1);
 await scenario('Other repeated day retains existing reset rule',[day(0,3),day(1,2),day(2,1),day(1)],5,1);
 await scenario('H I completion remains same calendar day',[0,1,2,3,4].map(n=>day(n)),5,5);await both('I reread unchanged completion',5,5);
 await scenario('J next calendar day clears completion',[0,1,2,3,4].map(n=>day(n,1)),5,0);
 await scenario('K next cycle saved day preserved',[...[0,1,2,3,4].map(n=>day(n,1)),day(0)],5,1);
 await scenario('Extra saved day on completion day remains capped',[...[0,1,2,3].map(n=>day(n,1)),day(4),day(0)],5,5);
 await scenario('Extra saved day survives next calendar date',[...[0,1,2,3].map(n=>day(n,2)),day(4,1),day(0,1)],5,1);
 await history([day(0)]);const persisted=(await captured())[0];
 await test('L duplicate UUID rejected and never increments',async()=>{const r=await req('client','/rest/v1/workouts',persisted);assert(r.status>=400);});await both('L unchanged after retry',5,1);
 await test('M invalid save rejected',async()=>{const r=await req('client','/rest/v1/workouts',{user_id:c.users.client.id,workout_date:date(0),data:{routine_id:c.routine,routine_day_id:c.unknown},id:'invalid'});assert(r.status>=400);});await both('M unchanged after failed write',5,1);
 await history([day(0),day(1)]);const editRows=await captured();
 await ok('client','/rest/v1/workouts?id=eq.'+editRows[0].id,{data:{...editRows[0].data,exercises:[{exercise_id:crypto.randomUUID(),sets:[{kg:0,reps:8,rir:2}],notes:'SYNTHETIC edit'}]}},'PATCH');await both('History result edit preserves identity and cycle',5,2);
 await ok('client','/rest/v1/workouts?id=eq.'+editRows[0].id,undefined,'DELETE');await both('History delete rereads remaining cycle',5,1);
 ...
 try{await ok('otherTrainer','/rest/v1/routine_days?id=eq.'+c.foreignDay,undefined,'DELETE');await check('Zero-day routine',0,0,'otherTrainer',c.users.otherTrainer.id,c.foreign);}finally{await ok('otherTrainer','/rest/v1/routine_days',{id:c.foreignDay,routine_id:c.foreign,name:'FOREIGN',day_order:0});}
 await history([{id:c.foreignDay,name:'FOREIGN'}],'otherClient',c.foreign);await check('One-day complete routine',1,1,'otherClient',c.users.otherClient.id,c.foreign);
